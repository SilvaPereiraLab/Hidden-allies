...
#packages and checks the version
...
path = "C:/single_spore_cultures/Data/Sequences"
list.files(path) #important to check if all files are present


#Generate matched lists of the forward and reverse read files
fnFs <- sort(list.files(path, pattern = "_R1_sub.fastq", full.names = TRUE))
fnRs <- sort(list.files(path, pattern = "_R2_sub.fastq", full.names = TRUE))
...
filterAndTrim(fnFs, fnFs.pre_filt, multithread = FALSE, trimLeft = 19, trimRight=20)
filterAndTrim(fnRs, fnRs.pre_filt, multithread = FALSE, trimLeft = 19, trimRight=20)
...
rbind(FWD.ForwardReads = sapply(FWD.orients, primerHits, fn = fnFs.pre_filt[1:9]), 
      FWD.ReverseReads = sapply(FWD.orients, primerHits, fn = fnRs.pre_filt[1:9]), 
      REV.ForwardReads = sapply(REV.orients, primerHits, fn = fnFs.pre_filt[1:9]), 
      REV.ReverseReads = sapply(REV.orients, primerHits, fn = fnRs.pre_filt[1:9]))
#primers in all sequences

#Inspect the quality profiles of the reads
plotQualityProfile(fnFs.pre_filt[1:9]) #forward
plotQualityProfile(fnRs.pre_filt[1:9]) #reverse
...
isolate <- c("Af_CI.01", "Af_CI.01", "Af_CI.01","Af_CI.02", "Af_CI.02","Af_CI.02","Af_CI.03","Af_CI.03", "Af_CI.03")
colony <- c("C1", "C2", "C3", "C1", "C2", "C3", "C1", "C2", "C3")
samdf <- data.frame(Isolate=isolate, Colony=colony)
...
#load("~/single_spore_cultures/Data/16S_taxid_table_output_dada2.RData")
#to load this dataframe 

#Save each separate ps dataframe as csv file - tax_table, refseq, abund_table, sam_table
write.csv(ps@tax_table,"~/single_spore_cultures/Data/CSV_original_files\\tax_table.csv", row.names = TRUE)
write.csv(ps@refseq,"~/single_spore_cultures/Data/CSV_original_files\\refseq.csv", row.names = TRUE)
write.csv(ps@otu_table,"~/single_spore_cultures/Data/CSV_original_files\\abund_table.csv", row.names = TRUE)
write.csv(ps@sam_data,"~/single_spore_cultures/Data/CSV_original_files\\sam_data.csv", row.names = TRUE)
